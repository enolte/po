...
namespace po
{
  ...
  constexpr auto operator*(E1&& expr1, E2&& expr2)
  {
    if constexpr(is_scalar<E2>)
      return expr_binary_mult<E1, expr_constant>{std::move(expr1), expr_constant{expr2}};
    else if constexpr(is_scalar<E1>)
      return expr_binary_mult<expr_constant, E2>{expr_constant{expr1}, std::move(expr2)};
    else
      return expr_binary_mult<E1, E2>{std::move(expr1), std::move(expr2)};
  }
}
