...
namespace po_test
{
  namespace rank3_degree3_dim3_lagrange_simplex_basis
  {
    static constexpr std::uint64_t rank = 3;
    static constexpr std::uint64_t degree = 3;
    static constexpr std::uint64_t simplex_dim = 3;
  ...
  void test_lagrange_basis_rank3_degree3_dim3_simplex()
  {
    using namespace rank3_degree3_dim3_lagrange_simplex_basis;
  ...
  namespace rank3_degree3_dim3_lagrange_simplex_basis
  {
    ...
    void test(const double vertices[][rank], int k)
    {
      ...
      PO_ASSERT(basis.size() == 20, basis.size());

      // Just a basic sanity check
      static constexpr double tol = 0x1p-31;
